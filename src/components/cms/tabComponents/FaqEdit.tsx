...
const FaqEdit = ({
  ...
  setSelectedFaq: (value: Faq | undefined) => void;
}) => {
  ...
  const handleUpdate = async (values: FieldValues) => {
    setLoading(true);
    try {
      const res = await axios.post(`/api/cms/faq/edit/${selectedFaq?.id}`, {
        ...values,
      });
  ...
  const handleDelete = async (id: string) => {
    try {
      const res = await axios.delete(`/api/cms/faq/delete/${selectedFaq?.id}`);
  ...
  return (
    <div className="min-w-[350px] border-[1px] border-[#d1d1d1] gap-4 flex-1 overflow-y-auto h-full hiddenscrollbar">
      <div className="h-[139px] w-full border-b-[1px] border-[#d1d1d1] lg:pr-[40px] md:pr-[40px] pr-[20px] pl-6 flex flex-row items-center justify-between">
        <span>Editing {selectedFaq?.question}</span>
        <IconButton onClick={() => setSelectedFaq(undefined)}>
          <Close />
        </IconButton>
      </div>
